* Introduction
  ...
  The SR Model Trainer (SMT) repository is a library for developing
  networks for Synaptics vision chips.  It consists of PyTorch model
  components which can be exported to TensorFlow and tflite without an
  intermediate export backend like ONNX.  The resulting tflite files
  are clean, and respect chip memory constraints.  The aim of SMT is
  to streamline the process of generating trained models to deploy on
  Synaptics chips, for internal and external use.

  SMT aims to support several "backends".  For now, the only backend
  supported in Ultralytics.  Ultralytics provides a suite of tools for
  training vision models and visualizing those models.  Using
  Ultralytics as a backend, you can generate trained vision models
  optimized for our chips.
...
** Planned for later releases
   ...
   - Models optimized for SL series of chips
...
* Installation
  ...
  #+begin_src shell
    pip install "sr-model-trainer[ultra] @ git+https://github.com/syna-astra-mcu-dev/sr-model-trainer.git"
  #+end_src

  or if you have cloned to a local copy of the repository:

  #+begin_src shell
    pip install [-e] "/PATH/TO/LOCAL/SR-MODEL-TRAINER[ultra]"
  #+end_src
...
* Quickstart
  ...
  In the following, I give a simple example of how one can train a
  model on COCO, quantize to tflite, and benchmark that tflite on a
  custom dataset specified by a user's CUSTOM_DATA.yaml.  First, train
  the model:
  #+begin_src shell
    smt ultralytics train model=sr100-detect-vga.yaml data=coco.yaml
  #+end_src
  Quantize the trained model.
  #+begin_src shell
    smt quantize --weights runs/train/detect/weights/best.pt --data /path/to/coco.yaml
  #+end_src
  Evaluate that trained and quantized model.
  #+begin_src shell
    smt ultralytics val model=runs/train/detect/weights/best.tflite task=detect data=coco.yaml
  #+end_src
  If you have a custom evaluation dataset, you can evaluate on that
  (e.g. test split) as well
  #+begin_src shell
    smt ultralytics val model=runs/train/detect/weights/best.tflite split=test task=detect save_txt=True save_conf=True data=CUSTOM_DATA.yaml
  #+end_src
...
** Ultralytics
   ...
   through SMT with SMT modules.  The basic shell syntax is:

   #+begin_src shell
     smt ultralytics [ultralytics ARGS]...
   #+end_src

   This performs 3 SMT-specific operations, then passes off execution
   to the normal Ultralytics code entrypoint:
   - Copy the model config from the SMT zoo if necessary.
   ...
   - Apply patches to the Ultralytics modules where necessary to
     enable proper SMT model loading within Ultralytics.
   ...
   After this point, you are free to use SMT models and tflites using
   the normal Ultralytics API, but do not try to use Ultralytics'
   "export" functionality to deploy to SR100.  Use SMT's quantize
   instead.  Otherwise, the resulting models will not be properly
   optimized and are not expected to run on our chips.
...
*** Train
    ...
    The SMT repository provides a thin wrapper around Ultralytics
    training for simple training situations.  The basic usage is

    #+begin_src shell
      smt ultralytics [OTHER ULTRALYTICS ARGS]
    #+end_src

    For instance, if you want to train a person detect model, you
    can train a VGA (640x480) model for the SR100 chip with.
...
*** Validation
    ...
    task, the dataset split, and the canvas size.  Currently,
    Ultralytics has the limitation that validation can only be done on
    a square canvas.  Hence, you should first quantize your model a
    seperate time with `--image-shape 640 640` and use that tflite
    version for validation.  Your validation invokation will look
    something like:

    #+begin_src shell
      smt ultralytics val model=runs/train/detect/weights/best.tflite split=val task=detect save_txt=True save_conf=True imgsz=640 data=coco.yaml
    #+end_src

    This should place the results of model evaluation in
    runs/val/detect.  For more information, see the ultralytics
...
*** Predict (for demos)
    ...
    #+begin_src shell
      smt ultralytics predict model=vga/detect/finetuned.tflite source=0 imgsz='[480,640]' show=True iou=.3 conf=.5
    #+end_src

    Breaking this apart: You are calling SMT with the ultralytics
...
* Core Shell API
  ...
  The basic syntax for running SMT from a shell is:

  #+begin_src shell
    smt [entrypoint] [entrypoint specific args]
  #+end_src

  Where entrypoint can be a native SMT module, or a backend like
  ultralytics.  For instance:

  #+begin_src shell
    smt ultralytics train ...
    smt quantize --backend ultralytics ...
  #+end_src
...
** Quantize
   ...
   The SMT repository includes the ability to quantize models

   #+begin_src shell
     smt quantize --backend BACKEND --weights MODEL_PT_SAVE --data REP_DATA
   #+end_src

   For instance, running:

   #+begin_src shell
     smt quantize --backend ultralytics --weights ./exp/weights/best.pt --data /PATH/TO/CUSTOM_DATASET.YAML --image-shape 480 640
   #+end_src

   will create a tflite at ./exp/weights/best.tflite with input shape
   [480, 640].  The image shape will default to whatever is defined on
   the original model yaml but can be overrided via command line as in
   the above.  You may also specify a model yaml like so:

   #+begin_src shell
     smt quantize --backend ultralytics --cfg sr100-detect-qvga.yaml
   #+end_src

   This will place a quantized model at ./sr100-detect-qvga.tflite.
...
** Metrics
   ...
   SMT's metrics code is an advanced model benchmarking tool which
   ...
   #+begin_src shell
     smt metrics DATA1.YAML DATA2.YAML... --out-dirs OUT_AIR1 OUT_DIR2... --project PLOT_DIR  --precisions PRECISION...
   #+end_src
   ...
   #+begin_src shell
     smt ultralytics val model=/PATH/TO/BEST.TFLITE split=test imgsz=HEIGHT,WIDTH data=DATA1.YAML task=detect save_txt=True save_conf=True
   #+end_src
...
*** Converting to Keras/TensorFlow
    ...
    SMT exists to be the glue between State of the Art training, and
    ...
    This method works so long as only SMT blocks operate directly on
...
** Test Suite
   ...
   installing SMT with the [dev] set of optional dependencies.

   #+begin_src shell
     pip install -e ...sr-model-trainer[dev]
   #+end_src
