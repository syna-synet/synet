...
* Installation
  ...
  install.  If you want the exact library versions used to produce the
  results in the Perforance section above, you can use the following
  (requires CUDA 11.8)
...
* Quickstart
  ...
  #+end_src
  Evaluate that trained model on your custom data test split.
  #+begin_src shell
...
*** Train
    ...
    #+begin_src shell
      synet ultralytics train model=sabre-keypoint-vga.yaml data=coco.yaml
    #+end_src
